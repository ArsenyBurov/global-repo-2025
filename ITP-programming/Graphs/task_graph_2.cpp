...
int main() {
    ...
    printlist(orig);
    cout << endl << "введите ребро, соединяющее A и B: ";
    cin >> newA >> newB; // Ребро для добавления: newA → newB
    int n;
    n = orig.size();
    // Проверка на корректность вершин (можно опустить, если входные данные гарантированно верны)
    if (newA < n && newB < n) {
        orig[newA].push_back(newB); // Добавляем ребро newA → newB
        cout << "ребро " << newA << " -> " << newB << "добавлено" << endl;
    }
    else {
        cout << "error" << endl;
    }
